export const Pizza = (props) => {
    ...
    const buttonSoldOut = (soldout) => {
        return soldout === 1 ? " bg-gray-700 disabled" : " hover:bg-blue-700";
    }
    ...
    const soldOutPrice = (soldout) => {
        return soldout === 1 ? " text-gray-500 line-through" : "";
    }
    ...
    return (
        <li key={props.pizza.id} className={"flex flex-col mt-4 text-center border-gray-500 border rounded rounded-xl" + soldOut(props.pizza.soldOut)}>
            ...
            <span className={"mt-4" + soldOutPrice(props.pizza.soldOut)}>Price: ${props.pizza.price}</span>
